...
#pragma pack(pop)
        ...
        static AxisAlignedResolvedTransformData ReolvedAxisAlignedTransformData(AxisAlignedTransform transform)
        {
            AxisAlignedResolvedTransformData transformData;

            transformData.flipX = (((int)transform.flip & (int)AxisAlignedFlip::Horizontal) != 0) ? 1 : 0;
            transformData.flipY = (((int)transform.flip & (int)AxisAlignedFlip::Vertical) != 0) ? 1 : 0;

            //if rotate is 90 counter clock wise flip horizontal axis
            switch (transform.rotation)
            {
            case AxisAlignedRotation::Rotate180:
                transformData.flipX ^= 1;
                transformData.flipY ^= 1;
                break;
            case AxisAlignedRotation::Rotate90CCW:
                transformData.flipY ^= 1;
                transformData.switchDimensions = 1;
                break;
            case AxisAlignedRotation::Rotate90CW:
                transformData.flipX ^= 1;
                transformData.switchDimensions = 1;
                break;
            case AxisAlignedRotation::None:
                break;
            }
